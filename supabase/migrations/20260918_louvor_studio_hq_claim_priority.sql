-- Uma preparação base atende toda a equipe e deve sair antes de uma versão
-- pessoal transposta. Assim, uma transposição pendente não bloqueia uma nova
-- música adicionada a uma escala.
...
BEGIN
  ...
  -- Prioridade 1: preparar a música base do set/culto.
  SELECT project_row.*
  INTO v_project
  ...
  -- Prioridade 2: versões individuais de músicas já preparadas.
  SELECT version_row.*
  INTO v_version
