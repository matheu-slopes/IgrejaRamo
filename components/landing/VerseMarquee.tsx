    ...
    <div className="flex shrink-0" aria-hidden>
      {verses.map((v, i) => (
        <span key={i} className="inline-flex items-center gap-3 px-10 shrink-0">
          ...
          <span className="font-sans text-gray-900 text-[0.95rem] font-light">{v.text}</span>
          <span className="font-sans text-[11px] tracking-[0.25em] text-gray-600 uppercase ml-1">{v.ref}</span>
        </span>
      ))}
    </div>
  );
...
export default function VerseMarquee() {
  return (
    <div className="relative overflow-hidden py-5 border-y border-gray-200/40">
